...
async function seed() {
	...
	const entities = ['user', 'match', 'chat']
	...
	console.time(`🐨 Created admin user "vadim"`)
	...
	await prisma.user.create({
		select: { id: true },
		data: {
			email: 'vadim@alexeev.co.uk',
			firstName: 'Vadim',
			lastName: 'Alexeev',
			image: { create: kodyImages.kodyUser },
			password: { create: createPassword('test123') },
			// connections: {
	...
	console.timeEnd(`🐨 Created admin user "vadim"`)
